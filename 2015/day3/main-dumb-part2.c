...
poslist_t santa_pos_list = {0, {0}};
poslist_t robos_pos_list = {0, {0}};
...
int check_houses(int turn, poslist_t *plists[], pos_t p) {
    if (check_house(plists[turn], p)) {
        return is_new_house(plists[turn^1], p);
    }
    return 0;
}
...
int main() {
    ...
    poslist_t *lists[2] = {&santa_pos_list, &robos_pos_list};
    ...
    do {
        ...
        switch(c) {
            case '>': current_pos[turn].x++; house_count += check_houses(turn, lists, current_pos[turn]); break;
            case '<': current_pos[turn].x--; house_count += check_houses(turn, lists, current_pos[turn]); break;
            case '^': current_pos[turn].y++; house_count += check_houses(turn, lists, current_pos[turn]); break;
            case 'v': current_pos[turn].y--; house_count += check_houses(turn, lists, current_pos[turn]); break;
