...
pub fn main() !void {
        ...
        while (j > 0) {
            ...
            if (bricks[j].isOver(bricks[i])) {
                ...
                if (gap == 1) {
                    ...
                    try bricks[i].supported_by_list.append(&bricks[j]);
    ...
    print("\nDetermine fallen bricks\n", .{});
    var total_fallen_bricks: usize = 0;
    for (bricks) |brick| {
        const brick_load = try brick.getSupportedLoad();
        total_fallen_bricks += brick_load;
        print("{:4} {}\n", .{ brick.id, brick_load });
    }
...
const Brick = struct {
    ...
    right: Coord,
    supported_by: usize,
    supported_load: ?usize,
    supported_by_list: std.ArrayList(*Brick),
    ...
    pub fn init(allocator: std.mem.Allocator, id: usize, x1: usize, y1: usize, z1: usize, x2: usize, y2: usize, z2: usize) Brick {
        return .{
            ...
            .supported_by = 0,
            .supported_by_list = std.ArrayList(*Brick).init(allocator),
            .supported_load = null,
    ...
    pub fn getSupportedLoad(self: Brick) !usize {
        // List of potential nodes to remove
        var removal_list = std.ArrayList(*const Brick).init(self.allocator);
        defer removal_list.deinit();

        // List to hold "removed" nodes (since we are not actually removing them)
        var removed_list = std.AutoHashMap(usize, void).init(self.allocator);
        defer removed_list.deinit();

        try removal_list.append(&self); // Add root brick to remove
        try removed_list.put(self.id, {});

        // Process removal list until empty
        while (removal_list.popOrNull()) |brick| {
            //print("Removing {}\n", .{brick.id});

            for (brick.supporting_list.items) |child| {
                //print("  Child {}\n", .{child.id});
                if (removed_list.contains(child.id)) {
                    //print("    Skip {}\n", .{child.id});
                    continue; // skip if already in removed list
                }
                var supported_by: usize = 0;
                for (child.supported_by_list.items) |parent| {
                    //print("    Parent {} {}", .{ parent.id, &parent });
                    if (!removed_list.contains(parent.id)) {
                        supported_by += 1;
                    } //else print(" in removed list", .{});
                    // print("\n", .{});
                }
                if (supported_by < 1) {
                    //print("    Added {}\n", .{child.id});
                    try removal_list.insert(0, child);
                    try removed_list.put(child.id, {});
                }
            }
        }

        return removed_list.count() - 1; // minus one since not counting self in supported load
    }
};
