...
pub fn main() !void {
    ...
    while (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        // Get the direction
        if (split_line.next()) |dir| {
            try expect(dir.len == 1);
            op.dir = switch (dir[0]) {
                'R' => .right,
                'L' => .left,
                'D' => .down,
        ...
        // Get dig distance
        if (split_line.next()) |dist| {
            op.dist = try std.fmt.parseInt(u8, dist, 10);
        } else return error.MissingInput;

        // Get RGB code
        if (split_line.next()) |rgb| {
            op.rgb.red = try std.fmt.parseInt(u8, rgb[2..4], 16);
            op.rgb.green = try std.fmt.parseInt(u8, rgb[4..6], 16);
            op.rgb.blue = try std.fmt.parseInt(u8, rgb[6..8], 16);
        } else return error.MissingInput;
    ...
    var area = areaVertexList(vertex_list.items);
